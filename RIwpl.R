...
###uninformative 
estimator_PL<- fit_PL(y=StrSRSWORSample$y, g=StrSRSWORSample$cluster, x=StrSRSWORSample$x, pars=c(truevalue[1:2], log(truevalue[3:4])))
estimator_PL

###informative sampling
estimatoris_PL<- fit_PL(StrSRSWORSampleis$y, StrSRSWORSampleis$cluster, StrSRSWORSampleis$x, pars=c(truevalue[1:2], log(truevalue[3:4])))
estimatoris_PL
...
fit_WPL<-function(y,g,x, pos, sc, n2infor, N2,  pars){
   ...
   gr<-function(theta){
      ...
      wincrementds=exp(theta[3])*wdsigma2(y[i],y[j],g[i],g[j],x[i],x[j],
                                        alpha=theta[1],beta=theta[2],sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[i], pos[j], sc[i], sc[j], n2infor,N2)
      wincrementdt=exp(theta[4])*wdtau2(y[i],y[j],g[i],g[j],x[i],x[j], alpha=theta[1],beta=theta[2],
                                      sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[i], pos[j], sc[i], sc[j], n2infor,N2)
...
estimator_WPL<- fit_WPL(StrSRSWORSample$y, StrSRSWORSample$cluster,StrSRSWORSample$x, StrSRSWORSample$ID_unit, 
                       StrSRSWORSample$strata,n2infor=rep(n2,N1), N2,  pars=c(truevalue[1:2], log(truevalue[3:4])))
estimator_WPL
...
estimatoris_WPL<- fit_WPL(StrSRSWORSampleis$y, StrSRSWORSampleis$cluster,StrSRSWORSampleis$x, StrSRSWORSampleis$ID_unit, 
                            StrSRSWORSampleis$strata,n2infor=n2is, N2,  pars=c(truevalue[1:2], log(truevalue[3:4])))
estimatoris_WPL
...
pairscore_WPL<-function(y,g,x, theta, pos, sc, n2infor, N2){
   ...
   incrementda=wdalpha(y[i],y[j],g[i],g[j],x[i],x[j], alpha=theta[1],beta=theta[2],
                       sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[i], pos[j],sc[i], sc[j], n2infor, N2)
   incrementdb=wdbeta(y[i],y[j],g[i],g[j],x[i],x[j],alpha=theta[1],beta=theta[2],
                      sigma2=exp(theta[3]),tau2=exp(theta[4]),pos[i], pos[j], sc[i], sc[j], n2infor, N2)
   incrementds=exp(theta[3])*wdsigma2(y[i],y[j],g[i],g[j],x[i],x[j],
                                      alpha=theta[1],beta=theta[2],sigma2=exp(theta[3]),tau2=exp(theta[4]),pos[i], pos[j],  sc[i], 
                                      sc[j], n2infor, N2)
   incrementdt=exp(theta[4])*wdtau2(y[i],y[j],g[i],g[j],x[i],x[j], alpha=theta[1],beta=theta[2],
                                    sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[i], pos[j], sc[i], sc[j], n2infor, N2)
   ...
   for (i in 1:(n-1)){
      ...
      for(j in js){
         ...
         incrementdaij=wdalpha(y[i],y[j],g[i],g[j],x[i],x[j], alpha=theta[1],beta=theta[2],sigma2=exp(theta[3]),
                               tau2=exp(theta[4]), pos[i], pos[j],sc[i], sc[j],n2infor,N2)
         ...
         incrementdsij=exp(theta[3])*wdsigma2(y[i],y[j],g[i],g[j],x[i],x[j],alpha=theta[1],beta=theta[2],
                              sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[i], pos[j],sc[i], sc[j], n2infor,N2)
         incrementdtij=exp(theta[4])*wdtau2(y[i],y[j],g[i],g[j],x[i],x[j], alpha=theta[1],beta=theta[2],sigma2=exp(theta[3]),
                              tau2=exp(theta[4]),pos[i], pos[j], sc[i], sc[j], n2infor,N2)
         wpsij=c(incrementdaij, incrementdbij, incrementdsij, incrementdtij)
         ...
         incrementdakl=wdalpha(y[k],y[l],g[k],g[l],x[k],x[l], alpha=theta[1],beta=theta[2],
                               sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[k], pos[l], sc[k], sc[l],n2infor,N2)
         ...
         incrementdskl=exp(theta[3])*wdsigma2(y[k],y[l],g[k],g[l],x[k],x[l],alpha=theta[1],beta=theta[2],
                              sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[k],pos[l],sc[k], sc[l], n2infor,N2)
         incrementdtkl=exp(theta[4])*wdtau2(y[k],y[l],g[k],g[l],x[k],x[l], alpha=theta[1],beta=theta[2],
                                            sigma2=exp(theta[3]),tau2=exp(theta[4]), pos[k], pos[l], sc[k], sc[l], n2infor,N2)
         wpskl=cbind(incrementdakl, incrementdbkl, incrementdskl, incrementdtkl)
...
for(i in 1:LOTS){
   ...
   rb<-fit_PL(StrSRSWORSample$y,StrSRSWORSample$cluster, StrSRSWORSample$x,pars=c(1,1,0,0) )
   rc<-fit_WPL(StrSRSWORSample$y,StrSRSWORSample$cluster,StrSRSWORSample$x, StrSRSWORSample$ID_unit, 
                    StrSRSWORSample$strata, n2infor=rep(n2, N1), N2,  pars=c(1,1,1,1))
   
   #NML, PL and WPL (informative sampling)
   rais<-lmer(y~(1|cluster)+x,data=StrSRSWORSampleis)
   rbis<-fit_PL(y=StrSRSWORSampleis$y,g=StrSRSWORSampleis$cluster, x=StrSRSWORSampleis$x,pars=c(1,1,1,1) )
   rcis<-fit_WPL( y=StrSRSWORSampleis$y,g=StrSRSWORSampleis$cluster,
                       x=StrSRSWORSampleis$x, StrSRSWORSampleis$ID_unit,StrSRSWORSampleis$strata, n2infor=n2is, N2, pars=c(1,1,1,1))
   ...
   PSis_PL[i, ]<- pairscore_PL(y=StrSRSWORSampleis$y,g=StrSRSWORSampleis$cluster,
                               x=StrSRSWORSampleis$x,theta=c(truevalue[1:2], log(truevalue[3:4])))
...
color=c( rep(c("green", "blue", "red", "yellow"), 4))
name=c("alpha_NML", "beta_NML", "sigma^2_NML", "tau^2_NML", "alpha_PL", "beta_PL", "sigma^2_PL", "tau^2_PL", "alpha_WPL", "beta_WPL", "sigma^2_WPL", "tau^2_WPL" )
boxplot(rmb[, c(1:4, 5:8, 9:12)],   col=color)
abline(h=0)
#boxplot for informative sampling (NML,PL and WPL)
boxplot(rmb[,c(13:16 ,17:20 ,21: 24)],   col=color)
abline(h=0)
